...
function BinCreator({ onCreateBin, maxHeight }) {
  ...
  const [binConfig, setBinConfig] = useState({
    ...
    height: 1,
    label: '',
    color: null, // null means use default color based on type

    // Hollow bin properties
    wallThickness: 1.2,
    stackable: false,
    magnets: false,
    lid: false,
    dividers: { horizontal: 0, vertical: 0 },
    fingerGrabs: false,

    // Solid bin properties
    cutout: null
  });
  ...
  // Load available cutout files
  useEffect(() => {
    // In a real implementation, you would fetch this from the public/cutouts directory
    // For now, we'll use the ones mentioned in the README
    setAvailableCutouts([
